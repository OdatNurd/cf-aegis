...
function createMiniflareOptions(config) {
  ...
  if (config.queues?.producers) {
    // CORRECT MAPPING: Miniflare expects a Record<string, string> of binding to queue name.
 ...
 */
 export async function aegisSetup(ctx, config) {
  ...
  try {
    ...
    // Get simple key-value, asset, and queue bindings from the generic getter.
