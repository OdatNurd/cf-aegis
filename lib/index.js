...
export async function aegisSetup(ctx, inputConfig, helperOptions = {}) {
  ...
  const { portAdjustment = 0, workerMocks = {} } = helperOptions;
  ...
  try {
    ...
    if (APPLY_ASSET_WORKAROUND) {
      applyAssetRouterWorkaround(miniflareOptions.workers);
    }

    // Ensure every worker has the CF_AEGIS binding set to 'true', unless
    // it was already defined by the configuration.
